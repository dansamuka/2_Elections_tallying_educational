...
| OCR/matching accuracy | Fixed (Banissa); Ol Kalou startup-safety fixed | Portal-label and OCR-text stream disambiguation now recognize IEBC's real "NAME NN" / "POLLING STATION X of Y" conventions (previously only matched a "Stream N" keyword that doesn't appear on real forms/portal labels). Fixed a rejected-ballots label that never matched the real form text, added derived `total_cast`. Fixed `"auto"` OCR mode, which meant "Tesseract only" regardless of configured cloud credentials -- now prefers GCV/Textract when configured. Added `google-cloud-vision`/`boto3` to the `historical-ocr` extra (previously only in `[ocr]`, never installed by the sync workflow). **Critical fix on the Ol Kalou side:** `Dockerfile` only installed `[s3]`, never `[ocr]` -- turning on `OCR_MODE=dual-cloud` without this would have crashed the live worker at startup, election night. `build_extractor()` now fails open to `NoOpExtractor` instead of crashing worker startup on any OCR construction failure. See `OCR_AND_MATCHING_ACCURACY_NOTES.md`. |
| Save & mark reviewed / confirm tally | Implemented, both pipelines | Archive dashboard: explicit confirm button turns a stream's grid cell green, updates a local tally banner, auto-advances to the next unconfirmed stream. Ol Kalou review console: tally strip reusing the existing `/api/provisional` endpoint, refreshed after every double-entry submission. See `OCR_AND_MATCHING_ACCURACY_NOTES.md`. |
